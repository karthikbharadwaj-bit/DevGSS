...
* Description...........: Case 04977978                                                           *
*                         Change Implementation_Status_2__c to SCHEDULE if booking bug            *
*                         created an event and Implementation_Status_2__c existing value = NEW    *
*                         Else, do not change Implementation_Status_2__c value                    * 
**************************************************************************************************/

trigger Event on Event (before insert, before update) {
    
    if(TriggerHandler.BY_PASS_EVENT_ON_INSERT || TriggerHandler.BY_PASS_EVENT_ON_UPDATE){ 
    ...
    if(useridSet != null && useridSet.size()>0) {
        ...
        // Case 04977978 - Add Implementation_Status_2__c in the Query        
        for(Implementation__c implemetationObj : [SELECT RC_USER_ID__c, Account__c, Account__r.Id, Implementation_Status_2__c 
    ...
    System.debug('Event Record Type test' + ((SObject)Type.forName('Event').newInstance()).getSObjectType().getDescribe().getRecordTypeInfosByName());
    Id SERecType = ((SObject)Type.forName('Event').newInstance()).getSObjectType().getDescribe().getRecordTypeInfosByName().get('SE Event').getRecordTypeId();
    String implementationKeyPrefix = ((SObject)Type.forName('Implementation__c').newInstance()).getSObjectType().getDescribe().getKeyPrefix();
    for(Event evet:trigger.new){
        try{
            if(trigger.isInsert){
                if(mapImplementation != null && !String.isBlank(evet.CustomerId__c) && mapImplementation.get(evet.CustomerId__c) != null) { 
                    ...
                    if((evet != null  && UserInfo.getName().equalsIgnoreCase('Booking Bug') && evet.Appointment_Source__c.contains('Calendar' )) || ( evet.Type != null && evet.Type.equalsIgnoreCase(INITIAL_IMPLEMENTATION)) ){
        ...
        if(mapImplementation != null && !string.isBlank(evet.CustomerId__c) && mapImplementation.get(evet.CustomerId__c) != null) { 
            ...
            if((evet != null && UserInfo.getName().equalsIgnoreCase('Booking Bug') &&  evet.Appointment_Source__c.contains('Calendar')) || (evet.Type != null && evet.Type.contains('Follow-Up Implementation') && !evet.Type.contains('System Use')) ){
            /*evet != null && evet.Type != null && !evet.Type.contains('Follow-Up Implementation') && !evet.Type.contains('System Use')*/
                system.debug('+++++++++++++++++++++++ddddddddddd_______'+evet);
                Implementation__c implementationObj = new Implementation__c(Id = mapImplementation.get(evet.CustomerId__c).Id);

                // Case 04977978 - Set to Schedule if Existing Status = New
                // implementationObj.Implementation_Status_2__c = 'Scheduled';
                if(mapImplementation.get(evet.CustomerId__c).Implementation_Status_2__c == 'New'){
                    implementationObj.Implementation_Status_2__c = 'Scheduled';
                }
                // Case 04977978 - Set to Schedule if Existing Status = New

                system.debug('+++++++++++++++++++++++_______'+implementationObj.Implementation_Status_2__c);
     ...
     } catch(Exception e){evet.addError(e);}
  }
    ...
    try {
        ...
        PlatformLog.logException('Event.trigger: opportunity update', e);
    }
    try {
        ...
        PlatformLog.logException('Event.trigger: lead update', e);
    }
