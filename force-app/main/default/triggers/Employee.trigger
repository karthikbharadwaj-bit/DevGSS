trigger Employee on Employee__c (before insert,before update)
{
    if(trigger.isInsert) {
         ...
         if(userIds.size() != 0) {
         	// Modified by Viren on 16th Nov for Channel__c
            userMap = new Map<Id, User>([select id,Department,Division,Team__c, Channel__c from User where id IN:userIds ]);
         }
         ...
         for(Employee__c emp:accs) { 
             if(emp.User__c != null ) {
                 ...
                 if(user != null) {
                     ...
                     user.Channel__c = emp.Channel__c;
