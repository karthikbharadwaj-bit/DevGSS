trigger PartnerCommAfterTgr on Partner_Communication__c (after insert, after update) {
    ...
    if(Trigger.isInsert){
        try{
            ...
            String pcmPrefixId = ((SObject) Type.forName('Partner_Communication__c').newInstance()).getSObjectType().getDescribe().getKeyPrefix();
            for(Partner_Communication__c pcm : Trigger.new){
                if(pcm.ConnectionReceivedId == NULL ){// local PC records and should be shared with TELUS
                    PartnerNetworkRecordConnection newConnection = new PartnerNetworkRecordConnection(
                       ConnectionId = pcm.ConnectionIdOfPartner__c, 
                    ...
                    CaseComment newComment = new CaseComment (CommentBody = pcm.Vendor_CommentBody__c , ParentId = (String.valueOf(pcm.Case__c) != NULL ? String.valueOf(pcm.Case__c) : pcm.Case_Id__c), IsPublished = pcm.Is_Public__c);
            ...
            for(Partner_Communication__c pcm : [SELECT Id,case__c,Case_Comment_ID__c,Case_Id__c FROM Partner_Communication__c WHERE Id IN : pcCommToCaseCommentMap.keySet()]){
                if(pcCommToCaseCommentMap != NULL && pcCommToCaseCommentMap.containsKey(pcm.Id)){
                    ...
                    pcm.Case_Comment_ID__c = pcCommToCaseCommentMap.get(pcm.Id).Id; 
                    if(pcm.case__c == NULL){
