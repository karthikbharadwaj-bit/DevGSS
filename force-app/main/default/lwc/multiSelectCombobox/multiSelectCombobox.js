...
export default class MultiSelectCombobox extends LightningElement {
  ...
  set options(value) {
    let options = [];
    if (value && value.length > 0 && this.selectAllOption) {
      options = [{label: "Select All", value: "selectAll", selected: true}];
    }
    this._options = [...options, ...value];
    this.currentOptions = JSON.parse(JSON.stringify(this._options));
  ...
  /**
   * If present, the select all option is added at the beginning.
   * @type {boolean}
   * @default false
   */
  ...
  change(event) {
    ...
    if (event.detail.item.value === 'selectAll') {
        this.currentOptions
      ...
      if (this.selectAllOption) {
        const selectAllOption = this.currentOptions.find(item => item.value === "selectAll");

        const hasFalseSelected = this.currentOptions.some(item => item.selected === false);
        if (hasFalseSelected) {
        ...
        const allOptionsSelected = this.currentOptions.every(item => item.value === "selectAll" || item.selected);
  ...
  setSelection() {
    ...
    } else if (selectedItems.length > 2 && selectedItems.length < this.currentOptions.length && this.selectAllOption) {
      const selectAllOption = this.currentOptions.find(item => item.value === "selectAll");
      selection = (selectAllOption && selectAllOption.selected ? `${selectedItems.length - 1} ` : `${selectedItems.length} `) + 'Options Selected';
      this.selectedOptions = this.getSelectedItems();
    } else if (selectedItems.length > 2 && selectedItems.length < this.currentOptions.length) {
