...
const GENERIC_ERROR = 'Company Intelligence is temporarily unavailable. Try again later.';
...
const SECTION_DEFINITIONS = [
    ...
    { key: 'executiveLeadershipAnnouncements', title: 'Executive Leadership Announcements' },
    { key: 'businessAndFinancialSignals', title: 'Business and Financial Signals' },
    { key: 'competitiveAndMarketContext', title: 'Competitive and Market Context' },
...
export function buildCompanyViewModel(intelligence = {}) {
    ...
    const sections = SECTION_DEFINITIONS.map((definition) => ({
        ...
        items: toStrings(summary[definition.key]).map((text, index) => ({
    ...
    const sources = (Array.isArray(intelligence.sources) ? intelligence.sources : [])
        ...
        .filter((source) => source.label && source.url && source.index !== null && source.index !== undefined);
    const sourceIndexToSourceMap = new Map(sources.map((source) => [String(source.index), source]));

    const claims = (Array.isArray(intelligence.derivedInformation)
        ...
        .map((claim, claimIndex) => ({
            ...
            sources: (Array.isArray(claim && claim.sourceIndices) ? claim.sourceIndices : [])
                .map((sourceIndex) => sourceIndexToSourceMap.get(String(sourceIndex)))
                .filter(Boolean)
                .map((source) => ({ ...source, key: `claim-${claimIndex}-source-${source.index}` }))
        }))
    ...
    return {
        ...
        hasSearchSuggestionsMarkup: Boolean(
            intelligence.searchEntryPoint && intelligence.searchEntryPoint.renderedContent
        )
    };
}
...
export default class CompanyIntelligence extends LightningElement {
    ...
    get showEmptyState() {
        return !this.isLoading && !this.isError && this._hasRequested && !this.hasContent;
    }
    ...
    async requestIntelligence() {
        ...
        try {
            const result = await requestCompanyIntelligence({ recordId: this.recordId });
