...
import generateSummary from '@salesforce/apex/AISummaryController.generateLeadSummary';
import logAIHEvent from '@salesforce/apex/AISummaryController.logAIHEvent';
import { ShowToastEvent } from 'lightning/platformShowToastEvent';

const SUBFEATURE = 'Lead Summary';

export default class LeadAISummary extends LightningElement {
    @api recordId;
    ...
    @wire(getRecord, { recordId: '$recordId', fields: [LEAD_NAME, LEAD_COMPANY] })
    wiredLead({ data }) {
        this.leadName = data ? (getFieldValue(data, LEAD_NAME) || '') : '';
        this.leadCompany = data ? (getFieldValue(data, LEAD_COMPANY) || '') : '';
    }

    get visiblePoints() { return this.bulletPoints; }

    connectedCallback() {
        // start collapsed (like Account/Case)
        this.activeSections = [];
    }

    closeAccordion = () => { this.activeSections = []; };
    ...
    _buildReactionPayload(choice) {
        return {
            feature: 'LeadSummary',
            parentNumber: this.leadCompany || this.leadName || this.recordId,
            itemType: 'Lead',
