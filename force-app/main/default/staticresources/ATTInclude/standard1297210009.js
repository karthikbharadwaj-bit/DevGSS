...
{var result=arg[0];if(result.success===1&&!RightNow.UI.Form.logoutInProgress)
{RightNow.UI.Form.logoutInProgress=true;if(result.socialLogout)
RightNow.Url.navigate(result.socialLogout,true);else if(this.data.attrs.redirect_url==='')
RightNow.Url.navigate(result.url,true);else
RightNow.Url.navigate(this.data.attrs.redirect_url+result.session,true);}},_onLogoutClick:function()
...
RightNow.Widget.LoginDialog2=function(data,instanceID){this.data=data;this.instanceID=instanceID;this._dialog=this._keyListener=null;this._errorDisplay=document.getElementById("rn_"+this.instanceID+"_ErrorMessage");this._container=document.getElementById("rn_"+this.instanceID)
if(this._container)
...
RightNow.UI.addDevelopmentHeaderError("Error with LoginDialog2 widget, trigger_element attribute value was set to '"+this.data.attrs.trigger_element+"', but an element with that ID doesn't exist on the page.");RightNow.Event.subscribe("evt_loginFormSubmitResponse",this._onResponseReceived,this);}};RightNow.Widget.LoginDialog2.prototype={_onLoginTriggerClick:function(type,arg){if(!this._dialog){var dialogTitle=this.data.attrs.label_dialog_title;var buttons=[{text:this.data.attrs.label_login_button,handler:{fn:this._onSubmit,scope:this},isDefault:true},{text:this.data.attrs.label_cancel_button,handler:{fn:this._onCancel,scope:this},isDefault:false}];this._dialog=RightNow.UI.Dialog.actionDialog(dialogTitle,document.getElementById("rn_"+this.instanceID),{"buttons":buttons});YAHOO.util.Dom.addClass(this._dialog.id,'rc_MiniLogin_DialogBox');this._keyListener=RightNow.UI.Dialog.addDialogEnterKeyListener(this._dialog,this._onSubmit,this);this._dialog.validate=function(){return false;};}
if(this._errorDisplay)
this._errorDisplay.innerHTML="";YAHOO.util.Dom.removeClass("rn_"+this.instanceID,'rn_Hidden');this._dialog.cancelEvent.subscribe(this._onCancel,null,this);this._dialog.show();var focusElement=null;this._usernameField=document.getElementById("rn_"+this.instanceID+"_Username");this._passwordField=document.getElementById("rn_"+this.instanceID+"_Password");if(this._usernameField)
{if(this._usernameField.value==='')
focusElement=this._usernameField;else
focusElement=this._passwordField;RightNow.UI.Dialog.enableDialogControls(this._dialog,this._keyListener,focusElement);focusElement.focus();}},_onCancel:function(type,arg){if(this._errorDisplay)
{this._errorDisplay.innerHTML="";YAHOO.util.Dom.removeClass(this._errorDisplay,'rn_MessageBox rn_ErrorMessage');YAHOO.util.Dom.addClass(this._dialogElement,"rn_Hidden");}
if(this._dialog)
...
errorMessage=RightNow.Interface.getMessage("USERNAME_FIELD_CONTAIN_SPACES_MSG");else if(eo.data.username.indexOf("'")>-1||eo.data.username.indexOf('"')>-1)
errorMessage=RightNow.Interface.getMessage("USERNAME_FIELD_CONT_QUOTE_CHARS_MSG");else if(eo.data.username.indexOf("<")>-1||eo.data.username.indexOf(">")>-1)
errorMessage=RightNow.Interface.getMessage("USERNAME_FIELD_CONTAIN_THAN_CHARS_MSG");if(errorMessage!=="")
...
return false;},_onResponseReceived:function(type,result){result=result[0];new YAHOO.util.Anim("rn_"+this.instanceID+"_Content",{opacity:{to:1}},0.5,YAHOO.util.Easing.easeIn).animate();YAHOO.util.Dom.removeClass("rn_"+this.instanceID,'rn_ContentLoading');if(this.instanceID==result.w_id)
{if(result.success==1)
...
{this._addErrorMessage(result.message,'rn_'+this.instanceID+'_Username');}}},_getRedirectUrl:function(result){if(this.data.js.redirectOverride)
{var sessionValue=result.sessionParm.substr(result.sessionParm.lastIndexOf("/")+1);if(!sessionValue&&this.data.js.redirectSession)
sessionValue=this.data.js.redirectSession;return RightNow.Url.addParameter(this.data.js.redirectOverride,'session',sessionValue)+this.data.attrs.append_to_url;}
if(this.data.attrs.redirect_url==='')
{if(result.addSession)
return result.url+this.data.attrs.append_to_url+result.sessionParm;return result.url+this.data.attrs.append_to_url;}
if(result.addSession)
return this.data.attrs.redirect_url+result.sessionParm;return this.data.attrs.redirect_url;},_addErrorMessage:function(message,focusElement){if(this._errorDisplay)
{YAHOO.util.Dom.addClass(this._errorDisplay,'rn_MessageBox rn_ErrorMessage');YAHOO.util.Dom.removeClass(this._dialogElement,"rn_Hidden");this._errorDisplay.innerHTML='<a href="javascript:void(0);" onclick="document.getElementById(\''+focusElement+'\').focus(); return false;">'+message+'</a>';this._errorDisplay.firstChild.focus();}}};
...
if(this._errorDisplay)
{this._errorDisplay.innerHTML="";YAHOO.util.Dom.removeClass(this._errorDisplay,'rn_MessageBox rn_ErrorMessage');YAHOO.util.Dom.addClass(this._errorDisplay,'rn_Hidden');}
...
{YAHOO.util.Dom.removeClass(this._errorDisplay,'rn_MessageBox rn_ErrorMessage');YAHOO.util.Dom.addClass(this._errorDisplay,'rn_Hidden');this._errorDisplay.innerHTML="";var returnValue=true;if(this._emailField)
...
{var confirmSend=RightNow.UI.Dialog.messageDialog(arg[0],{icon:"WARN",exitCallback:{fn:this._enableDialog,scope:this},width:'448px'});YAHOO.util.Dom.addClass(confirmSend.id,'rn_SiteFeedback2Dialog rn_ConfirmSend');}
else
{var confirmSend=RightNow.UI.Dialog.messageDialog(this.data.attrs.label_feedback_confirmation,{exitCallback:{fn:this._closeDialog,scope:this},width:'448px'});YAHOO.util.Dom.addClass(confirmSend.id,'rn_SiteFeedback2Dialog rn_ConfirmSend');}}},_addErrorMessage:function(message,focusElement)
{if(this._errorDisplay)
{YAHOO.util.Dom.addClass(this._errorDisplay,'rn_MessageBox rn_ErrorMessage');YAHOO.util.Dom.removeClass(this._errorDisplay,'rn_Hidden');var newMessage='<a href="javascript:void(0);" onclick="document.getElementById(\''+focusElement+'\').focus(); return false;">'+message+'</a>';var oldMessage=this._errorDisplay.innerHTML;if(oldMessage==="")
