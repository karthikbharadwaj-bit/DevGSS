...
RightNow.Widget.DisplaySearchFilters=function(data,instanceID){this.data=data;this.instanceID=instanceID;if(this.data.js.filters)
{for(var i=0;i<this.data.js.filters.length;i++)
{YAHOO.util.Event.addListener("rn_"+this.instanceID+"_Remove_"+i,"click",this._onFilterRemove,i,this);}}
else
{this.data.js.filters=[];}
this._resetData=[];this._filters=this.data.js.filters;delete this.data.js.filters;this._defaultFilters=this.data.js.defaultFilters;delete this.data.js.defaultFilters;RightNow.Event.subscribe("evt_searchFiltersResponse",this._updateFilters,this);RightNow.Event.subscribe("evt_menuFilterRequest",this._updateFilters,this,true);RightNow.Event.subscribe("evt_resetFilterRequest",this._resetFilters,this);RightNow.Event.subscribe("evt_reportResponse",this._displayFilters,this);};RightNow.Widget.DisplaySearchFilters.prototype={_updateHierFilter:function(existingFilter,updatedFilter)
{this._updateHierFilter._createHierFilter=this._updateHierFilter._createHierFilter||function(newFilter){if(newFilter.data)
return{"label":newFilter.data.label,"level":newFilter.data.level,"hierList":newFilter.filters.data[0].join(",")};else
return newFilter.filters.data.reconstructData;};if(!existingFilter)
{var label=(updatedFilter.filters.searchName==="p")?RightNow.Interface.getMessage("PRODUCT_LBL"):RightNow.Interface.getMessage("CATEGORY_LBL");existingFilter=this._createFilter(updatedFilter,label);}
if(updatedFilter.data)
{if(updatedFilter.data.value===0)
{existingFilter.data=[];}
else
{if(updatedFilter.data.level>existingFilter.data.length)
{while(updatedFilter.data.level>existingFilter.data.length+1)
existingFilter.data.push(existingFilter.cache[existingFilter.data.length]);if(!this._updateFromReport||!existingFilter.cache.length)
existingFilter.data.push(this._updateHierFilter._createHierFilter(updatedFilter));else
existingFilter.data.push(existingFilter.cache.pop());}
else
{while(existingFilter.data.length>updatedFilter.data.level)
existingFilter.data.pop();if(updatedFilter.data.level>0)
{if(existingFilter.data.length===0)
{existingFilter.data.push(this._updateHierFilter._createHierFilter(updatedFilter));}
else
{if(!this._updateFromReport)
existingFilter.data[existingFilter.data.length-1]=this._updateHierFilter._createHierFilter(updatedFilter);}}}
if(!existingFilter.cache||existingFilter.data.length>existingFilter.cache.length)
existingFilter.cache=existingFilter.data.slice();}
existingFilter.touched=true;}
else if(updatedFilter.filters.data.reconstructData)
{existingFilter.data=updatedFilter.filters.data.reconstructData;existingFilter.touched=true;}
return existingFilter;},_updateFilter:function(existingFilter,updatedFilter)
{if(!existingFilter)
existingFilter=this._createFilter(updatedFilter);if(existingFilter.data!==updatedFilter.filters.data.label)
{existingFilter.data=updatedFilter.filters.data.label;existingFilter.filterID=(existingFilter.type!=="org")?updatedFilter.filters.fltr_id:updatedFilter.filters.data.selected;existingFilter.touched=true;}
this._checkDefaultFilterValues(existingFilter);return existingFilter;},_createFilter:function(newFilter,label)
{this._createFilter._createLabel=this._createFilter._createLabel||function(newFilter,label){var finalLabel=label||newFilter.filters.label;if(!finalLabel)
...
return finalLabel;};return{"filterID":newFilter.filters.fltr_id||newFilter.filters.data.selected,"name":newFilter.filters.searchName,"type":newFilter.filters.rnSearchType,"label":this._createFilter._createLabel(newFilter,label),"cache":[],"data":[],"touched":true};},_checkDefaultFilterValues:function(filterToCheckFor)
{for(var i=0;i<this._defaultFilters.length;i++)
{if(this._defaultFilters[i].name===filterToCheckFor.name)
{if(filterToCheckFor.filterID==this._defaultFilters[i].filterID)
{filterToCheckFor.removeDefault=true;filterToCheckFor.touched=false;}}}},_updateFilters:function(evt,eventObj,updatingHierarchyType)
{eventObj=eventObj[0];if(eventObj&&eventObj.filters.report_id===this.data.attrs.report_id&&(eventObj.filters.fltr_id||(eventObj.filters.data&&eventObj.filters.data.fltr_id)))
{var filterUpdated=false;for(var i=0,index;i<this._filters.length;i++)
{if(this._filters[i]&&this._filters[i].name===eventObj.filters.searchName)
{this._resetData[i]=RightNow.Lang.cloneObject(this._filters[i]);if(updatingHierarchyType)
this._filters[i]=this._updateHierFilter(this._filters[i],eventObj);else if(eventObj.filters.rnSearchType!=="menufilter")
this._filters[i]=this._updateFilter(this._filters[i],eventObj);filterUpdated=true;break;}}
if(!filterUpdated&&eventObj.filters.searchName)
{if(updatingHierarchyType&&(eventObj.filters.data.reconstructData||(eventObj.data&&eventObj.data.value)))
this._filters[this._filters.length]=this._updateHierFilter(null,eventObj);else if(eventObj.filters.rnSearchType!=="menufilter")
this._filters[this._filters.length]=this._updateFilter(null,eventObj);}}},_resetFilters:function(evt,args)
{if(args[0].data.name==="all"&&RightNow.Event.isSameReportID(args,this.data.attrs.report_id))
{for(var i=0;i<this._filters.length;i++)
{if(this._resetData[i])
this._filters[i]=RightNow.Lang.cloneObject(this._resetData[i]);}}},_displayFilters:function(evt,args)
{var reportFilters=args[0].filters.allFilters.filters;for(var filter in reportFilters)
{if(reportFilters[filter]&&reportFilters[filter].filters&&(reportFilters[filter].filters.fltr_id||(reportFilters[filter].filters.data&&reportFilters[filter].filters.data.fltr_id)))
{this._updateFromReport=true;var updatingHier=false;if(reportFilters[filter].filters.rnSearchType==="menufilter")
{updatingHier=true;if(reportFilters[filter].data)
{if(typeof reportFilters[filter].filters.data[0]==="string")
reportFilters[filter].filters.data[0]=reportFilters[filter].filters.data[0].split(",");else
reportFilters[filter].filters.data[0]=RightNow.Lang.arrayFilter(reportFilters[filter].filters.data[0]);reportFilters[filter].data.level=(reportFilters[filter].filters.data[0])?reportFilters[filter].filters.data[0].length:0;}
else
{reportFilters[filter].filters.searchName=filter;}}
else
{if(reportFilters[filter].filters.data&&reportFilters[filter].filters.data.val)
{reportFilters[filter].filters.fltr_id=reportFilters[filter].filters.data.val;reportFilters[filter].filters.searchName=filter;}
else
{reportFilters[filter].filters.fltr_id=reportFilters[filter].filters.data;}}
this._updateFilters(null,[reportFilters[filter]],updatingHier);}}
this._updateFromReport=false;this._displayFilters._filtersRemoved=0;var outputChanged=false,widgetElement=document.getElementById("rn_"+this.instanceID);for(var i=0,index;i<this._filters.length;i++)
{if(this._filters[i]&&this._filters[i].touched)
{var newElement=this._buildFilterElement(i,this._filters[i].label),filterData=this._filters[i].data;if(filterData&&this._filters[i].type==="menufilter")
{if(filterData.length)
{for(var j=0;j<filterData.length;j++)
{newElement.innerHTML+=this._buildFilterItemMarkup(filterData[j],null,(j===filterData.length-1),this._filters[i].name);}}
else
{newElement.innerHTML="";this._displayFilters._filtersRemoved++;}}
else if(filterData)
{newElement.innerHTML+=this._buildFilterItemMarkup(this._filters[i],filterData,true);}
else
{newElement.innerHTML="";this._displayFilters._filtersRemoved++;}
if(widgetElement&&newElement)
{var outdatedFilter=document.getElementById(newElement.id);if(outdatedFilter)
{if(newElement.innerHTML==="")
outdatedFilter.parentNode.removeChild(outdatedFilter);else
...
{var existingElement=document.getElementById("rn_"+this.instanceID+"_Filter_"+i);if(existingElement)
existingElement.parentNode.removeChild(existingElement);this._displayFilters._filtersRemoved++;this._filters[i].removeDefault=false;}}
if(this._displayFilters._filtersRemoved===this._filters.length)
YAHOO.util.Dom.addClass("rn_"+this.instanceID,"rn_Hidden");else if(outputChanged)
YAHOO.util.Dom.removeClass("rn_"+this.instanceID,"rn_Hidden");},_buildFilterItemMarkup:function(item,label,shouldBeSelected,filterName)
{var cssClass=(shouldBeSelected)?"rn_Selected":"",filter=label||item.label,link=(!shouldBeSelected&&item.hierList)?RightNow.Url.addParameter(this.data.js.searchPage,filterName,item.hierList):"javascript:void(0);";filter="<a href='"+link+"' class='"+cssClass+" rn_FilterItem'>"+filter+"</a>";filter+="<span class='rn_Separator "+cssClass+"'/>";return filter;},_buildFilterElement:function(index,label)
{var filterElement=document.createElement("div"),title;filterElement.id="rn_"+this.instanceID+"_Filter_"+index;YAHOO.util.Dom.addClass(filterElement,"rn_Filter");title="<div class='rn_Label'>"+label+"<a id='rn_"+this.instanceID+"_Remove_"+index+"' href='javascript:void(0);' title='"+this.data.attrs.label_filter_remove+"'> ";if(this.data.attrs.remove_icon_path)
title+="<img alt='"+this.data.attrs.label_filter_remove+"' src='"+this.data.attrs.remove_icon_path+"'/>";else
title+=this.data.attrs.label_filter_remove;title+="</a></div>";filterElement.innerHTML=title;return filterElement;},_onFilterRemove:function(evt,index)
{this._onFilterRemove._removedFilters=this._onFilterRemove._removedFilters||0;YAHOO.util.Event.stopEvent(evt);var eo=new RightNow.Event.EventObject();eo.data={"name":this._filters[index].name};eo.filters={"report_id":this.data.attrs.report_id};eo.w_id=this.instanceID;RightNow.Event.fire("evt_resetFilterRequest",eo);eo.filters.reportPage=window.location.pathname;RightNow.Event.fire("evt_searchRequest",eo);if(!this._filters[index].cache)
this._filters[index].cache=this._filters[index].data;else if(this._filters[index].data.length!==this._filters[index].cache.length)
this._filters[index].cache.reverse();this._filters[index].data=[];var filterElement=document.getElementById("rn_"+this.instanceID+"_Filter_"+index);if(filterElement)
{filterElement.innerHTML="";filterElement.parentNode.removeChild(filterElement);}
...
word="<a href='"+RightNow.Url.addParameter(this.data.js.linkUrl+encodeURIComponent(word.replace(/\&amp;/g,"&"))+parameterList,"session",RightNow.Url.getSession())+"'>"+word+"</a>";resultQuery+=word+" ";}
...
{var topicWordItems=eventObject.data.topic_words;for(var i=0;i<topicWordItems.length;i++)
{var displayIcon=(this.data.attrs.display_icon)?topicWordItems[i].icon:"";topicWordsHtml+="<dt>"+displayIcon+"<a href='"+topicWordItems[i].url+"' target='"+this.data.attrs.target+"' />"+topicWordItems[i].title+"</a></dt><dd>"+topicWordItems[i].text+"</dd>";}
...
{link=document.createElement("span");YAHOO.util.Dom.addClass(link,"rn_CurrentPage");if(i===startPage)
{YAHOO.util.Dom.addClass(link,"rn_FirstPaginatorElement");}
link.innerHTML=i;}
else
{link=document.createElement("a");link.id="rn_"+this.instanceID+"_PageLink_"+i;if(i===startPage)
{YAHOO.util.Dom.addClass(link,"rn_FirstPaginatorElement");}
link.href=this.data.js.pageUrl+i;link.innerHTML=i;titleString=this.data.attrs.label_page;if(titleString)
